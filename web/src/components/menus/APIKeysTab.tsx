...
export const APIKeysTabContent = memo(function APIKeysTabContent({
  ...
  // The secrets list includes unconfigured registry placeholders (`is_configured:
  // false`). Only count keys that actually have a stored value — otherwise
  // multi-field providers (Google Mail, DataForSEO, GitHub OAuth) look Connected
  // because both of their field keys appear in the list.
  const configuredKeys = useMemo(
    () =>
      new Set(
        safeSecrets.filter((s) => s.is_configured).map((s) => s.key)
      ),
    [safeSecrets]
  );

  // Fully configured providers for the Connected section. Unconfigured cards
  // come from PROVIDER_META below so registry placeholders don't double-render.
  const connected = useMemo(() => {
    ...
    for (const secret of safeSecrets) {
      ...
      if (!areAllFieldsConfigured(meta, configuredKeys)) {
        continue;
      }
  ...
  const hasContent = useMemo(() => {
    ...
    for (const sectionKey of SECTION_ORDER) {
      if ((unconfiguredBySection[sectionKey] || []).length > 0) return true;
    }
    return false;
  ...
  return (
    <FlexColumn sx={{ gap: "1.5rem" }}>
      ...
      {SECTION_ORDER.map((sectionKey) => {
        ...
        const allInSection = unconfigured.map((meta) => ({
          secret: {
            key: meta.key,
            is_configured: false,
            description: meta.description,
